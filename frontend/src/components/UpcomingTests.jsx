...
import { useGetUpcomingTestsQuery, useDeleteTestByIdMutation } from '../redux/api/testApiSlice.js'
...
const UpcomingTests = () => {
    ...
    const {data: testsData, refetch, isLoading, isError} = useGetUpcomingTestsQuery(id);
    ...
    const handleEditClick=(e, testId)=>{
          ...
          <div className="max-h-[400px] overflow-y-scroll overflow-x-scroll">
            <table className="min-w-full text-center">
              ...
              <tbody className="text-black text-sm">
                {testsData.tests.map((test, index) => {
                  ...
                  return (
                    <tr key={index} className="border-t">
                      ...
                      className=" cursor-pointer  px-2 text-xs py-1 bg-white border-4 border-[#f4f4f4]">
                        <div onClick={copyToClipboard} className='hover:border-dashed hover:border px-0'>{test.testCode}</div>
                      </td>
                      <td className="px-2 text-sm py-1 bg-white border-4 border-[#f4f4f4]">
                        <FaEdit 
                        onClick={(e)=>handleEditClick(e, test._id)}
                        className='mx-auto cursor-pointer text-gray-500 hover:text-black' />
                      </td>
                ...
                <OverlayModal isOpen={updateModalOpen} onClose={()=>setUpdateModalOpen(false)}>
                    <UpdateTest testId={testId} onClose={()=>setUpdateModalOpen(false)}/>
                </OverlayModal>
              </tbody>
            </table>
          </div>
